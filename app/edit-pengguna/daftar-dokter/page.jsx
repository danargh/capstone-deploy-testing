...
export default function DaftarDokter({ params }) {
  ...
  useEffect(() => {
    axios
      .get('https://647a44b3a455e257fa648a39.mockapi.io/penggunas/dokter')
                    ...
                    {PaginatedData().map((penggunas, i) => (
                          <tr scope="col" key={penggunas.id} className={selectedId === penggunas.id ? 'bg-gray-200' : 'bg-white'}>
                              ...
                              <td className="flex gap-3 py-2 justify-center border">
                                <button onClick={() => handlePrint(penggunas.id)} className="w-[68px] h-[35px] rounded-md  bg-web-green-300 text-white">Lihat</button>
                                <button onClick={() => handleDelete(penggunas.id)} className="w-[68px] h-[35px] rounded-md bg-red-800 text-white">Hapus</button>
                              </td>
                          </tr>
                    ))}      
                  </tbody>
