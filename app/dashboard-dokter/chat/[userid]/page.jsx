...
export default function Page({ params }) {
   ...
   const { chatHistoryData } = ChatHistoryAPI();
   useEffect(() => {
      chatHistoryData?.map((history) => {
         const { Content, Sender } = history;

         // Example: Update the chatHistory state with the new history item
         setChatHistory((prevChatHistory) => [
            ...prevChatHistory,
            { message: Content, type: Sender },
         ]);
      });
   }, [chatHistoryData]);

   const WSendpoint =
      "wss://capstone-project.duckdns.org:8080/doctor/chat/" + token;

   // Prevent double connection
   const socket = useRef(null);

   useEffect(() => {
      // Fetch History before conversation
      const createSocket = () => {
         const socket = new WebSocket(WSendpoint);

         // Event listener for Successfull WebSocket connection establishment
         ...
         // Event listener for WebSocket disconnections -- Auto Retry connections
