...
import { VWalletLists } from '@/components/ui/BankCardList';
import { useAtom } from 'jotai';
import useWithdrawal, { WithdrawalMethodAtom } from '@/components/atoms/useWithdrawal';
...
export default function BankForm() {
   ...
   const validationSchema = Yup.object().shape({
      ...
      amount: Yup.number().typeError('Nominal harus berupa angka').min(100000, 'Minimal Transfer Rp.100.000').required('Nominal harus diisi'),
   });
   ...
   function SelectedBankCard({ selectedBank }) {
      const selectedBankData = VWalletLists.find((bank) => bank.bankName === selectedBank);
      return (
         <div className="flex flex-row gap-[11px] items-center justify-start self-stretch shrink-0 relative">
            ...
            <div className="font-inter font-medium text-xs/[130%] text-[rgba(0,0,0,0.47)] text-left relative w-[107px]">{selectedBankData.bankName}</div>
         </div>
      );
   }
            ...
            <div className="flex flex-col gap-6 items-start justify-start shrink-0 relative">
               <div className="flex flex-col gap-5 items-start justify-start shrink-0 relative">
                  ...
                  <div className="flex flex-col gap-[19px] items-center justify-start shrink-0 relative">
                     <div className="flex flex-col gap-[19px] items-start justify-start shrink-0 relative">
                        <div className="flex flex-col gap-0 items-start justify-start shrink-0 relative">
                           <div className="pt-2.5 pr-0 pb-2.5 pl-0 flex flex-row gap-2.5 items-start justify-start shrink-0 relative">
                              <div className="font-inter font-medium font-md text-neutral-900 text-center relative">Nomor Rekening</div>
                           </div>
                           <input
                              type="text"
                           ...
                           {formik.touched.accountNumber && formik.errors.accountNumber ? (
                              <div className="font-inter font-light text-[15px]/[150%] text-[#ff0000] text-left relative w-[453px] flex items-center justify-start">{formik.errors.accountNumber}</div>
                           ) : null}
                        </div>
                        ...
                        <div className="flex flex-col gap-1 items-start justify-start shrink-0 relative">
                           <div className="flex flex-col gap-0 items-start justify-start shrink-0 relative">
                              <div className="pt-2.5 pr-0 pb-2.5 pl-0 flex flex-row gap-2.5 items-start justify-start shrink-0 relative">
                                 <div className="font-inter font-medium text-md text-neutral-900 text-center relative">Nominal</div>
                              </div>
                              <input
                                 type="text"
                              ...
                              {formik.touched.amount && formik.errors.amount ? (
                                 <div className="font-inter font-light text-[15px]/[150%] text-[#ff0000] text-left relative w-[453px] flex items-center justify-start">{formik.errors.amount}</div>
                              ) : null}
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="flex flex-col gap-0 items-start justify-start shrink-0 relative">
                        <div className="pt-2.5 pr-0 pb-2.5 pl-0 flex flex-row gap-2.5 items-start justify-start shrink-0 relative">
                           <div className="font-inter font-medium text-md text-neutral-900 text-center relative">Pesan (Opsional)</div>
                        </div>
               ...
               <button type="submit" className="font-poppins font-medium text-lg text-neutral-0 text-left bg-web-green-300 rounded-xl pt-4 pr-3 pb-4 pl-3 flex gap-2.5 items-center justify-center w-[168px] h-[75px] relative">
                  Lanjutkan
               </button>
            </div>
         </div>
