         ...
         <section className="relative">
            <div className="font-inter font-[500] text-[16px] leading-[20px] text-[#D1E5BB] bg-[#63863E] w-[313px] h-screen fixed z-50 top-0 left-0">
               ...
               <button
                  ...
                  className={`${
                     path === '/logout' ? 'bg-web-green-400 border-solid border-l-[5px] pt-[5px] pr-[5px] pb-[5px] pl-3' : ' '
                  } flex flex-row gap-2.5 items-start justify-start self-stretch shrink-0 relative mx-[20px]  hover:bg-[#FFFFFF33] py-3 my-3 w-full`}
                  prefetch={false}
               >
                  {path === '/login' ? <SidebarDarkLogOutIcon /> : <SidebarLogOutIcon />}
                  <div className={`font-inter ${path === '/' ? 'font-bold' : 'font-[500]'} text-[16px] text-[#D1E5BB] text-left relative mx-[20px] py-3`}>Keluar</div>
               </button>
            </div>
            <div>{children}</div>
         </section>
